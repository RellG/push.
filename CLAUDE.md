...
- Web Google sign-in uses the redirect flow with `authDomain: 'bussdown.space'` plus a Cloudflare Worker proxying `bussdown.space/__/*` to `push-d9e0b.firebaseapp.com` — required because modern browsers block the cross-origin redirect handler (silent `getRedirectResult()` null → onboarding loop). Don't revert `authDomain` and don't remove the Worker route; see `docs/google-auth-proxy.md`. Google sign-in works on bussdown.space only (not the legacy Render URL).
