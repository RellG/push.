...
- Web Google sign-in/link is **popup-first** (`signInWithPopup`/`linkWithPopup`), falling back to the redirect flow only when popups are impossible (`_popupUnsupportedCodes`). This requires `authDomain: 'bussdown.space'` plus a Cloudflare Worker proxying `bussdown.space/__/*` to `push-d9e0b.firebaseapp.com` — modern browsers block the cross-origin auth handler (that blocking is also why popups originally *seemed* broken and were wrongly abandoned for redirect in 2026-07), and iOS Safari/PWAs additionally drop the sessionStorage handoff the redirect flow needs. Don't revert `authDomain`, don't remove the Worker route, don't switch back to redirect-first; see `docs/google-auth-proxy.md`. Google sign-in works on bussdown.space only (not the legacy Render URL).
- **Auth debugging on devices without a console:** open `https://bussdown.space/?authdebug=1` — a SnackBar reports how the last redirect settled and the profile check (`redirect=<user> current=<user> profile.exists=<bool>(server|cache)`). The onboarding decision reads the profile with a server-first get + retries, and a signed-in Google user is never routed to onboarding on an inconclusive read (flaky networks made cache fallbacks misread returning users as new).
...
1. **Social** — a shared leaderboard shipped 2026-07-08 (`leaderboard/{uid}` public-stats docs, synced client-side by `leaderboardSyncProvider`; board readable by any signed-in user, entries writable only by their owner). Since 2026-07-12 it's a **season board**: published totals only count days on/after `leaderboardSeasonStartKey` (`lib/data/repositories/leaderboard_repository.dart`). To reset the race, bump that date — do NOT delete `leaderboard` docs (a 2026-07-12 admin delete made everyone "disappear" until each user next opened the app, since entries only republish client-side). Further social (challenges, friends) still needs a green light.
